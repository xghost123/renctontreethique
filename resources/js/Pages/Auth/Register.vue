        ...
        <div class="auth-content re-fade-up">
            ...
            <div class="auth-card">
                <div class="page-header">
                    <h1 class="page-title">Créer votre profil</h1>
                    <p class="page-subtitle">Rejoignez votre mosquée et commencez votre rencontre éthique</p>
                </div>
                ...
                <form @submit.prevent="submit" class="form">
                    <div class="form-item">
                        ...
                        <input id="name" v-model="form.name" type="text" required autofocus autocomplete="name" class="field-input" placeholder="Votre nom" />
                        <span v-if="form.errors.name" class="field-error">{{ form.errors.name }}</span>
                    </div>

                    <div class="form-item">
                        <label class="field-label" for="email">Email</label>
                        <input id="email" v-model="form.email" type="email" required autocomplete="username" class="field-input" placeholder="vous@exemple.fr" />
                        <span v-if="form.errors.email" class="field-error">{{ form.errors.email }}</span>
                    </div>

                    <div class="form-item">
                        <label class="field-label" for="mobile">Téléphone (optionnel)</label>
                        <input id="mobile" v-model="form.mobile" type="tel" autocomplete="tel" class="field-input" placeholder="+33 6 12 34 56 78" />
                    ...
                    <div class="form-item">
                        ...
                        <div class="password-wrapper">
                            <input id="password" v-model="form.password" :type="showPassword ? 'text' : 'password'" required autocomplete="new-password" class="field-input" placeholder="8 caractères minimum" />
                    ...
                    <div class="form-item">
                        ...
                        <input id="password_confirmation" v-model="form.password_confirmation" :type="showPassword ? 'text' : 'password'" required autocomplete="new-password" class="field-input" placeholder="••••••••" />
                        <span v-if="form.errors.password_confirmation" class="field-error">{{ form.errors.password_confirmation }}</span>
                    </div>

                    <button type="submit" class="submit-btn" :disabled="form.processing">
                        <span v-if="!form.processing">Créer mon compte — gratuit</span>
                ...
                <p class="auth-link">
                    Déjà inscrit ?
                    <Link :href="route('login')" class="auth-link-cta">Se connecter</Link>
                </p>
            </div>
        </div>
    </div>
...
.ornament { position: absolute; width: 140px; height: 140px; color: #1c4532; opacity: .06; pointer-events: none; }
.ornament-tl { top: -20px; left: -20px; }
.ornament-br { bottom: -20px; right: -20px; transform: rotate(180deg); }
.auth-content { width: 100%; max-width: 400px; display: flex; flex-direction: column; align-items: center; position: relative; z-index: 1; }
.auth-logo { display: flex; flex-direction: column; align-items: center; gap: 4px; margin-bottom: 20px; }
.auth-logo-mark { width: 44px; height: 44px; background: #1c4532; border-radius: 12px; display: flex; align-items: center; justify-content: center; margin-bottom: 8px; box-shadow: 0 4px 12px rgba(28, 69, 50, .25); }
.logo-icon { width: 22px; height: 22px; color: #c8a028; }
.auth-logo-title { font-family: 'Cormorant Garamond', Georgia, serif; font-size: 2rem; font-weight: 500; color: #1c4532; letter-spacing: -.01em; line-height: 1; margin: 0; }
.auth-logo-arabic { font-family: 'Cormorant Garamond', Georgia, serif; font-size: .95rem; color: #c8a028; font-weight: 400; letter-spacing: .08em; margin: 0; }
.divider-re { display: flex; align-items: center; gap: 10px; width: 100%; max-width: 200px; margin-bottom: 24px; color: #c8a028; }
.divider-re::before, .divider-re::after { content: ''; flex: 1; height: 1px; background: linear-gradient(to right, transparent, rgba(200, 160, 40, .5), transparent); }
.auth-card { width: 100%; background: #fff; border-radius: 20px; padding: 32px 28px; box-shadow: 0 1px 2px rgba(28, 69, 50, .04), 0 4px 16px rgba(28, 69, 50, .08), 0 16px 40px rgba(28, 69, 50, .06); border: 1px solid rgba(28, 69, 50, .08); }
...
.field-input { height: 44px; border-radius: 10px; border: 1.5px solid #E2DDD5; background: #fafaf8; font-family: 'Plus Jakarta Sans', sans-serif; font-size: .9rem; color: #1c1c1c; padding: 0 12px; width: 100%; transition: border-color .15s, box-shadow .15s, background .15s; }
...
.password-toggle { position: absolute; right: 12px; top: 50%; transform: translateY(-50%); width: 20px; height: 20px; color: #9ca3af; background: none; border: none; cursor: pointer; padding: 0; display: flex; align-items: center; }
.password-toggle:hover { color: #1c4532; }
.server-error { display: flex; align-items: center; gap: 8px; padding: 10px 14px; background: #fef2f2; border: 1px solid #FECACA; border-radius: 10px; font-size: .82rem; color: #dc2626; }
.error-icon { width: 16px; height: 16px; flex-shrink: 0; }
.submit-btn { width: 100%; height: 48px; background: #1c4532; color: #fff; border: none; border-radius: 12px; font-family: 'Plus Jakarta Sans', sans-serif; font-size: .9rem; font-weight: 600; letter-spacing: .02em; cursor: pointer; transition: background .15s, transform .1s, box-shadow .15s; box-shadow: 0 2px 8px rgba(28, 69, 50, .25); margin-top: 4px; display: flex; align-items: center; justify-content: center; gap: 8px; }
