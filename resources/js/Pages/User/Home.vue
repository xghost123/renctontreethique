    ...
    <div class="min-h-screen pb-16" style="background: linear-gradient(180deg, #FBF7F0 0%, #F5EEDD 100%)">
        ...
        <div class="relative max-w-6xl mx-auto px-5 pt-10">
            <div class="flex flex-wrap items-end justify-between gap-4 mb-8">
                <div>
                    <h1 class="font-display text-3xl md:text-4xl font-medium text-[#0f3a7d] leading-tight" style="font-family:'Cormorant Garamond',serif">Mon espace</h1>
            ...
            <template v-else>
                ...
                <div v-if="pending || !mosque" class="card-re p-12 text-center re-fade-up">
                    <div class="w-20 h-20 rounded-full bg-[#ff6b6b]/10 border-2 border-[#ff6b6b]/30 flex items-center justify-center mx-auto mb-5">
                        <svg class="w-9 h-9 text-[#ff6b6b]" viewBox="0 0 24 24" fill="currentColor"><path d="M7 2h10v5h2v2h-1v9h3v2H3v-2h3v-9H5V7h2V2z"/></svg>
                    </div>
                ...
                <template v-else>
                    <!-- ═══ MOSQUE BANNER ═══ -->
                    <div class="bg-[#0f3a7d] rounded-3xl p-8 mb-8 relative overflow-hidden shadow-xl" style="box-shadow: 0 12px 40px rgba(28,69,50,.25)">
                        <div class="pattern-re absolute inset-0 opacity-[.1]"></div>
                        <svg class="absolute -top-12 -right-12 w-48 h-48 text-[#ff6b6b] opacity-[.07]" viewBox="0 0 100 100" fill="currentColor"><path d="M50 0a50 50 0 1 0 50 50A50 50 0 0 0 50 0zm0 90a40 40 0 1 1 40-40 40 40 0 0 1-40 40zm0-60a20 20 0 1 0 20 20 20 20 0 0 0-20-20z"/></svg>
                        <div class="relative flex flex-wrap items-center gap-6">
                            <div class="w-16 h-16 rounded-2xl bg-[#ff6b6b]/15 border border-[#ff6b6b]/30 flex items-center justify-center">
                            ...
                            <div class="flex-1 min-w-[200px]">
                                <div class="text-[11px] uppercase tracking-[.2em] text-[#ff6b6b] font-semibold mb-1">Ma mosquée</div>
                                ...
                                <div class="text-sm text-[#ff6b6b]/70 mt-1">{{ mosque.city }}{{ mosque.country ? ', ' + mosque.country : '' }}</div>
                            </div>
                            <div class="flex gap-8 text-center">
                                <div v-for="s in stats" :key="s.label" class="min-w-[72px]">
                                    ...
                                    <div class="text-[11px] text-[#ff6b6b]/60 mt-0.5">{{ s.label }}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div v-if="incoming.length" class="mb-8">
                        <h3 class="font-display text-xl font-semibold text-[#0f3a7d] mb-4" style="font-family:'Cormorant Garamond',serif">
                            Demandes reçues
                            <span class="ml-2 inline-flex items-center justify-center min-w-6 h-6 px-1.5 rounded-full bg-[#ff6b6b] text-[#0D2218] text-xs font-bold align-middle">{{ incoming.length }}</span>
                        </h3>
                        <div class="space-y-3">
                            <div v-for="p in incoming" :key="p.id" class="card-re p-5 flex flex-wrap items-center gap-4 hover:shadow-md transition-all">
                                <div class="w-12 h-12 rounded-full bg-gradient-to-br from-[#0f3a7d] to-[#17a2b8] flex items-center justify-center text-[#E4B84A] font-display text-lg font-bold" style="font-family:'Cormorant Garamond',serif">{{ initials(p.sender) }}</div>
                                <div class="flex-1 min-w-[200px]">
                                ...
                                <div class="flex gap-2.5">
                                    <button @click="respondProposal(p.id, 'accept')" :disabled="acting[p.id]"
                                        class="bg-[#0f3a7d] hover:bg-[#163828] text-white text-xs font-semibold px-5 py-2.5 rounded-xl transition-all active:scale-[.98] disabled:opacity-50">
                    ...
                    <div class="mb-8">
                        <div class="flex items-center justify-between mb-4">
                            <h3 class="font-display text-xl font-semibold text-[#0f3a7d]" style="font-family:'Cormorant Garamond',serif">Membres de ma mosquée</h3>
                            <Link href="/app/members" class="text-xs font-semibold text-[#ff6b6b] hover:text-[#B18E20] transition-colors">Tout voir →</Link>
                        </div>
                        ...
                        <div v-if="members.length" class="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
                            <div v-for="m in members" :key="m.id" class="card-re overflow-hidden hover:shadow-xl transition-all hover:-translate-y-0.5 group">
                                <div class="h-1.5 bg-gradient-to-r from-[#0f3a7d] via-[#17a2b8] to-[#ff6b6b]"></div>
                                <div class="p-5">
                                    ...
                                    <button v-if="!m.has_pending_proposal" @click="sendProposal(m.user_id)" :disabled="acting['p' + m.user_id]"
                                        class="w-full bg-[#0f3a7d] hover:bg-[#163828] text-white text-xs font-semibold py-2.5 rounded-xl transition-all active:scale-[.98] disabled:opacity-50">
                    ...
                    <div v-if="isModerator && joinRequests.length" class="mb-8">
                        <h3 class="font-display text-xl font-semibold text-[#0f3a7d] mb-4" style="font-family:'Cormorant Garamond',serif">
                            ...
                            <span class="ml-2 inline-flex items-center justify-center min-w-6 h-6 px-1.5 rounded-full bg-[#ff6b6b] text-[#0D2218] text-xs font-bold align-middle">{{ joinRequests.length }}</span>
                        </h3>
                        <div class="card-re divide-y divide-[#0f3a7d]/[.04]">
                            <div v-for="r in joinRequests" :key="r.id" class="px-6 py-4 flex items-center gap-4">
                                <div class="w-10 h-10 rounded-full bg-[#0f3a7d]/[.07] flex items-center justify-center text-[#0f3a7d] font-bold text-sm">{{ initials(r.user) }}</div>
                                <div class="flex-1 text-sm text-[#374151]">{{ firstName(r.user) }} <span class="text-[#8A9680]">souhaite rejoindre</span></div>
                                <Link href="/app/moderation" class="text-xs font-semibold text-[#ff6b6b] hover:text-[#B18E20]">Modérer →</Link>
                            </div>
                        </div>
                    </div>
                </template>
