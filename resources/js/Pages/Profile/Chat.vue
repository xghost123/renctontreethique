...
<template>
    ...
    <div class="min-h-screen pb-16" style="background: linear-gradient(180deg, #FBF7F0 0%, #F5EEDD 100%)">
        ...
        <div class="relative max-w-6xl mx-auto px-5 pt-8">
            ...
            <h1 class="font-display text-3xl font-medium text-[#0f3a7d] mb-2" style="font-family:'Cormorant Garamond',serif">Conversations</h1>
            <p class="text-sm text-[#8A9680] mb-6">Chaque message est validé par un modérateur avant d'être remis</p>

            <div v-if="notice" class="mb-4 bg-[#FEFCF0] border border-[#ff6b6b]/40 text-[#8A6D12] px-4 py-3 rounded-xl text-sm">{{ notice }}</div>
            ...
            <div class="grid md:grid-cols-3 gap-5">
                ...
                <div class="bg-white rounded-2xl border border-[#0f3a7d]/[.06] shadow-sm overflow-hidden md:col-span-1">
                    <div class="px-4 py-3.5 border-b border-[#0f3a7d]/[.06]">
                        <div class="text-[11px] uppercase tracking-widest text-[#ff6b6b] font-semibold">Mes conversations</div>
                    </div>
                    <div class="divide-y divide-[#0f3a7d]/[.04] max-h-[560px] overflow-y-auto">
                        <button v-for="c in conversations" :key="c.id" @click="openConversation(c.other_id)"
                            :class="['w-full px-4 py-3.5 text-left hover:bg-[#F8F6F0] transition-colors', activeConv?.id === c.id ? 'bg-[#F8F6F0] border-l-2 border-[#0f3a7d]' : 'border-l-2 border-transparent']">
                            <div class="flex items-center gap-3">
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center justify-between">
                                        <span class="font-semibold text-[#0f3a7d] text-sm truncate">{{ c.other_name }}</span>
                                        <span v-if="c.unread > 0" class="bg-[#ff6b6b] text-white text-[10px] font-bold w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0">{{ c.unread }}</span>
                                    </div>
                ...
                <!-- Chat window -->
                <div class="bg-white rounded-2xl border border-[#0f3a7d]/[.06] shadow-sm overflow-hidden md:col-span-2 flex flex-col" style="min-height: 600px">
                    <template v-if="activeConv">
                        <!-- Header -->
                        <div class="px-5 py-3.5 border-b border-[#0f3a7d]/[.06] flex items-center gap-3 bg-[#F8F6F0]">
                            <div class="w-9 h-9 rounded-full bg-[#0f3a7d] text-white flex items-center justify-center text-xs font-bold">
                                {{ (conversations.find(c => c.id === activeConv.id)?.other_name || '?').charAt(0) }}
                            </div>
                            <div>
                                <div class="font-semibold text-[#0f3a7d] text-sm">{{ conversations.find(c => c.id === activeConv.id)?.other_name }}</div>
                                <div class="text-[10px] text-[#8A9680]">🕌 Membre de votre mosquée</div>
                            </div>
                            <div class="ml-auto text-[10px] bg-[#ff6b6b]/10 text-[#8A6D12] px-2.5 py-1 rounded-full font-medium">Messages modérés</div>
                        </div>
                        ...
                        <div class="chat-messages flex-1 overflow-y-auto px-5 py-5 space-y-3" style="max-height: 440px">
                            ...
                            <div v-for="m in messages" :key="m.id" :class="['flex', m.from_me ? 'justify-end' : 'justify-start']">
                                <div :class="['max-w-[75%] rounded-2xl px-4 py-2.5', m.from_me ? 'bg-[#0f3a7d] text-white rounded-br-sm' : 'bg-[#F0EDE6] text-[#374151] rounded-bl-sm']">
                        ...
                        <!-- Input -->
                        <div class="px-5 py-4 border-t border-[#0f3a7d]/[.06]">
                            <div class="flex gap-3">
                                ...
                                <button @click="sendMessage" :disabled="sending || !newMessage.trim()"
                                    class="bg-[#0f3a7d] hover:bg-[#163828] text-white px-5 rounded-xl transition-all disabled:opacity-40 flex items-center gap-1.5 text-sm font-semibold">
                            ...
                            <div class="text-[10px] text-[#8A9680] mt-2 flex items-center gap-1.5">
                                <svg class="w-3 h-3 text-[#ff6b6b]" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/></svg>
                    ...
                    <template v-else>
                        <div class="flex-1 flex flex-col items-center justify-center p-10 text-center">
                            <div class="w-16 h-16 rounded-full bg-[#0f3a7d]/[.06] flex items-center justify-center mb-4">
                                <svg class="w-8 h-8 text-[#0f3a7d]" viewBox="0 0 24 24" fill="currentColor"><path d="M20 2H4a2 2 0 0 0-2 2v18l4-4h14a2 2 0 0 0 2-2V4a2 2 0 0 0-2-2zm0 14H5.17L4 17.17V4h16v12zM7 9h10v2H7V9zm0 4h7v2H7v-2z"/></svg>
                            </div>
                            <h3 class="font-display text-xl font-medium text-[#0f3a7d] mb-2" style="font-family:'Cormorant Garamond',serif">Sélectionnez une conversation</h3>
