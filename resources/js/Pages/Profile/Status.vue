    ...
    <div class="min-h-screen flex items-center justify-center" style="background: linear-gradient(160deg, #FBF7F0 0%, #F5EEDD 50%, #F0E6D0 100%)">
        ...
        <div class="relative w-full max-w-md mx-5">
            <div class="text-center mb-6">
                <div class="inline-flex w-14 h-14 rounded-2xl bg-[#1C4532] shadow-lg items-center justify-center mb-4" style="box-shadow: 0 6px 20px rgba(28,69,50,.3)">
                ...
                <h1 class="font-display text-2xl font-medium text-[#1C4532]" style="font-family:'Cormorant Garamond',serif">Rencontre Éthique</h1>
            </div>

            <div v-if="status === 'loading'" class="card-re p-8 text-center">
                <div class="animate-spin w-8 h-8 border-3 border-[#1C4532] border-t-transparent rounded-full mx-auto mb-4"></div>
            ...
            <div v-else-if="status === 'pending_approval'" class="card-re p-8 text-center re-fade-up">
                ...
                <h2 class="font-display text-2xl font-medium text-[#1C4532] mb-2" style="font-family:'Cormorant Garamond',serif">Profil en cours de validation</h2>
                ...
                <div class="bg-[#F8F6F0] rounded-xl px-4 py-3 mb-5 text-sm text-[#374151]">
                    <span class="font-semibold text-[#1C4532]">Profil complété à {{ completion }}%</span>
                ...
                <div class="flex flex-col gap-2.5">
                    ...
                    <Link href="/" class="text-xs text-[#8A9680] hover:text-[#1C4532] transition-colors">Retour à l'accueil</Link>
                </div>
            </div>
            ...
            <div v-else-if="status === 'no_profile'" class="card-re p-8 text-center re-fade-up">
                ...
                <h2 class="font-display text-2xl font-medium text-[#1C4532] mb-2" style="font-family:'Cormorant Garamond',serif">Votre profil n'est pas encore créé</h2>
            ...
            <div v-else-if="status === 'active'" class="card-re p-8 text-center re-fade-up">
                ...
                <h2 class="font-display text-2xl font-medium text-[#1C4532] mb-2" style="font-family:'Cormorant Garamond',serif">Votre profil est actif !</h2>
                <p v-if="mosque" class="text-sm text-[#8A9680] mb-5">Vous faites partie de <strong class="text-[#1C4532]">{{ mosque.name }}</strong> — découvrez les membres.</p>
            ...
            <div v-else class="card-re p-8 text-center">
                ...
                <h2 class="font-display text-xl font-medium text-[#1C4532] mb-3">Une erreur est survenue</h2>
                <Link href="/login" class="text-xs text-[#C8A028] font-semibold">Se reconnecter</Link>
            </div>
        </div>
    </div>
</template>
