        ...
        <div v-show="showPopup">
            ...
            <transition leave-active-class="transition duration-100 ease-in" leave-from-class="opacity-100"
                ...
                <ListboxOptions static
                    class="popup_maritial_statuses_section absolute mt-1 h-60  overflow-auto rounded-md bg-white border border-gray-400 hover:border-gray-500 px-4 py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm">
                    ...
                    <template v-for="(maritial_status, maritial_status_key) in translations.biodata_form.deserved_biodata.deserved_maritial_statuses_options" :key="maritial_status.id">
                        ...
                        <ListboxOption @click="onClickMaritialStatusesItems" v-slot="{ active, selected }" :value="maritial_status_key" v-if="(gender == 'male' && maritial_status_key != 'widow_no_child' && maritial_status_key != 'widow_with_child' ) || (gender == 'female' && maritial_status_key != 'widower_no_child' && maritial_status_key != 'widower_with_child' ) || !gender" as="template" :disabled="maritial_status.unavailable" >
                            <li :class="[
...
<style>
.popup_maritial_statuses_section{
    width: 18rem;
    z-index: 999;
}
