...
const onClickJobTitlesItems = (e) => {
    ...
    emits('onSelectJobs', selectedJobTitles.value);
};
...
onMounted(() => {
    if( props.JobTitles != null ){
        if( props.JobTitles != '' ){
            selectedJobTitles.value = props.JobTitles.split(",");
            emits('onSelectJobs', selectedJobTitles.value);
        }
    }
});
    ...
    <Listbox v-model="selectedJobTitles" multiple>
        <ListboxButton @click="showPopup = !showPopup"
            ...
            <span class="block truncate leading-tight">
                <ListboxLabel>{{ translations.biodata_form.deserved_biodata.deserved_job_titles_title }}:</ListboxLabel>
                {{ selectedJobTitles.map((job_title) => job_title).join(',') }}
            </span>
        ...
        <div v-show="showPopup">
            ...
            <transition leave-active-class="transition duration-100 ease-in" leave-from-class="opacity-100"
                ...
                <ListboxOptions static
                    ...
                    <div v-if="selectedJobTitles.length > 0" @click="showPopup = false" class="okay_button px-4 py-2 font-semibold text-sm bg-cyan-500 text-white rounded-full shadow-sm flex justify-center w-12">
                        <CheckIcon class="h-5 w-5" aria-hidden="true" />
                        <span>{{ translations.biodata_form.multiple_selection_ok }}</span>
                    </div>

                    <ListboxOption @click="onClickJobTitlesItems" v-slot="{ active, selected }" v-for="(job_title, job_title_key) in translations.biodata_form.deserved_biodata.deserved_job_titles_options" :key="job_title_key" :value="gender == 'female' ? job_title.replace('শিক্ষক', 'শিক্ষিকা') : job_title" as="template" :disabled="job_title.unavailable" >
                        <li :class="[
