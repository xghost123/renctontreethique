...
const onClickView = (single_biodata) => {
    ...
    let biodata_update = props.biodata_updates.filter( single_biodata_update => {
        return single_biodata_update.user_id == single_biodata.user_id;
    });
...
const onClickApprove = (single_biodata) => {
    if(confirm("Are you sure?")){
        axios.post(route('backend.biodata.approve_edit_request', {
            csrf_token,
            user_id : single_biodata.user_id,
        }))
        .then((response) => {
            if( response.data ){
                emits('onUpdateAllBiodatas', response.data);
                dt.rows( function ( idx, data, node ) {
                    return data[2] == single_biodata.user_id ? true : false;
                }).remove().draw();
                modalInner.value = {
                    modalHeading : 'Success!',
                    modalDescription : 'The edit request approved successfully.',
                    showButtons : false
                }
                isPopupMessageModalOpen.value = true;
            }else{
...
const onClickCancel = (single_biodata) => {

    if(confirm("Are you sure?")){
        axios.post(route('backend.biodata.cancel_edit_request', {
            csrf_token,
            user_id : single_biodata.user_id,
        }))
        .then((response) => {
            if( response.data ){
                ...
                modalInner.value = {
                    modalHeading : 'Success!',
                    modalDescription : 'The edit request canceled successfully.',
                    showButtons : false
                }
                isPopupMessageModalOpen.value = true;
            }else{
...
document.body.classList.add("backend.approved.edit_requests");
        ...
        <DataTable v-if="all_biodatas.length > 0" ref="table" id="myTable" :options="options" class="display responsive  stripe row-border hover order-column nowrap" style="width:100%">
            ...
            <tbody>
                ...
                :key="single_biodata.id">
                    <tr v-if="single_biodata.in_edit_request && (single_biodata.is_approved || single_biodata.is_approved == 1) && (!single_biodata.in_admin_trash && single_biodata.in_admin_trash != 1)">
                        <td></td>
                        ...
                        <td>
                            <div class="flex flex-col sm:flex-row justify-center items-center gap-1">
                                ...
                                <button type="button" @click="onClickApprove(single_biodata)" class="action_button text-xs bg-blue-500 hover:bg-blue-700 !text-white font-bold py-2 px-4 rounded-full">
                                    Approve
                                </button>
                                <button type="button" @click="onClickCancel(single_biodata)" class="action_button text-xs bg-blue-500 hover:bg-blue-700 !text-white font-bold py-2 px-4 rounded-full">
                                    Cancel
                                </button>
                            </div>
                        </td>
